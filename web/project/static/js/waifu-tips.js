...
function initWidget(waifuPath, apiPath) {
	...
	if ((localStorage.getItem("waifu-display") && new Date().getTime() - localStorage.getItem("waifu-display") <= 86400000)) return;
	localStorage.removeItem("waifu-display");
	sessionStorage.removeItem("waifu-text");
	$("#drawer").append(`<div id="waifu">
			<div id="waifu-tips" class="mdui-typo"></div>
	...
	(function() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			else if (domain == 'baidu') text = 'Hello！来自 百度搜索 的朋友<br/>你是搜索 ' + referrer.search.split('&wd=')[1].split('&')[0] + ' 找到的我吗？';
			else if (domain == 'so') text = 'Hello！来自 360搜索 的朋友<br/>你是搜索 ' + referrer.search.split('&q=')[1].split('&')[0] + ' 找到的我吗？';
			else if (domain == 'google') text = 'Hello！来自 谷歌搜索 的朋友<br/>欢迎阅读『' + document.title.split(' - ')[0] + '』';
	...
	function showMessage(text, timeout, priority) {
		...
		if (!sessionStorage.getItem("waifu-text") || sessionStorage.getItem("waifu-text") <= priority) {
			...
			var brk = 13
			var len = text.length
			for (var i = 0; i * brk < len; i++) {
				for (var j = 0; j < brk; j++) {
					let chr = text.charAt(i*brk + j)
					if (!chr) break;
					if (chr === "『") {
						toShow += '<span style=\"color:#0099cc;\">『'
					} else if (chr === "』") {
						toShow += '』</span>'
					} else {
						toShow += chr;
			...
			$("#waifu-tips").stop().html(toShow).fadeTo(200, 1);
